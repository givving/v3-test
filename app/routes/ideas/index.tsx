...
export default function Index() {
  ...
  const [visibleItems, setVisibleItems] = useState<boolean[]>(Array(displayedItems.length).fill(false));
          ...
          <div className="w-full max-w-3xl px-4"> 
            <div className="flex flex-col items-center text-center">
              <div className="pt-12 pb-3 font-givving text-givving-primary max-w-lg">
                Gift ideas for a woman who's 31 – 40, lives in New Zealand and likes horse riding, hyrox, yoga, investing and sci-fi, with a budget of $201 – 300
              </div>
              <span 
            ...
            {displayedItems.map((item, index) => (
              <div 
                ...
                onClick={() => {
                  // navigate("/results")
                  navigate("/results", { state: { data: testData, index: index } });
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xl md:text-3xl font-givving text-givving-primary font-semibold">{item.group}</p>
                  </div>
                </div>
              </div>
            ))}
            ...
            <div className="flex justify-start mt-6 mb-12 w-full">
              {displayCount < testData.results.length ? (
                <Button 
                  ...
                  className="rounded-full border-blue-500 text-blue-500 font-bold hover:bg-blue-500 hover:text-white transition-colors px-6"
                >
                  SHOW MORE IDEAS
                </Button>
              ) : (
                <p className="text-givving-primary text-sm font-bold">All ideas shown</p>
              )}
            </div>
          </div>
        </div>
